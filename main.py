...
class DBNET():
    def __init__(self):
        self.sess = rt.InferenceSession('./model/ocr/dbnet.onnx')
...
class CRNNHandle:
    def __init__(self):
        self.sess = rt.InferenceSession('./model/ocr/crnn_lite_lstm.onnx')
...
class faster_rcnn():
    def __init__(self, confidence=0.5, threshold=0.3, use_nms=False):
        ...
        self.net = cv2.dnn.readNet('./model/ocr/faster_rcnn.pb', './model/ocr/faster_rcnn.pbtxt')
...
if __name__ == "__main__":
    id_img = cv2.imdecode(np.fromfile(r'C:\Users\jisuanji\Desktop\idcard\qitongwei.jpg', dtype=np.uint8), -1) #身份证照片
    # id_img = cv2.rotate(id_img, cv2.ROTATE_90_COUNTERCLOCKWISE)
    img = cv2.imdecode(np.fromfile(r'C:\Users\jisuanji\Desktop\材料\证件照.jpg', dtype=np.uint8), -1) #现场照片
    face_recognitio = FaceRecognition()

    # 人脸识别  demo.jpg  C:\Users\jisuanji\Desktop\IDcard.jpg  transformed_image: (1, 3, 32, 2)
    print("检测ing")
    faces_img = list()
    ...
    ide,_ = face_recognitio.feature_compare(results[0]["embedding"], results2[0]["embedding"], 0.7)   # default = 0.7
    if ide:
        print('同一人')
    else:
        print('不匹配')

    srcimg = cv2.flip(id_img,0)
    myocr = OCR()
    results = myocr.det_rec(srcimg)

    # 翻转匹配
    match = re.search(r'姓名(.+)', results[0]['text'])
    if match is None:
    ...
    for i, res in enumerate(results):
        ...
        #     idname = res['text'].split(' ')[1]
        #     print("身份证号码:",idname)
        # for j in range(4):
        #     cv2.circle(srcimg, tuple(point[j, :]), 3, (0, 255, 0), thickness=-1)  results[i]['text']
        print(res['text'])

    ################ 在这里写个pandas，将results里面的姓名和证件号提取出来保存为df，然后在存为excel
